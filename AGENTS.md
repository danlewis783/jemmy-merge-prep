...
- **Windows first.** Only `gradlew.bat` is committed; the Unix `gradlew` script is deliberately
  gitignored. On Linux or macOS, run the committed wrapper jar directly. It downloads Gradle
  9.7 like `./gradlew` would, and doesn't rewrite repo files the way `gradle wrapper` does:
...
- Run under a virtual display:
  `xvfb-run -a -s "-screen 0 1920x1080x24" java -cp gradle/wrapper/gradle-wrapper.jar org.gradle.wrapper.GradleWrapperMain check --continue`.
