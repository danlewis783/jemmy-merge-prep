...
| Mnemonic | Resolution |
|---|---|
| `factory-construction` | Done 2026-07-08 — operators are no longer constructed via searching constructors that block inside `this(...)` chains. Each operator class now exposes `of(component)` (direct wrap) and `waitFor(...)` factories (search-and-wait, formerly the searching constructors); the direct-hit constructor is package-private, callable only from the factories and subclass constructors inside the operators package. ~2,700 call sites migrated. Caveat: `waitFor`/`of` are statics, so a subclass that omits an overload silently inherits the superclass variant returning the supertype — declare the full overload set when adding operators |
| `reflective-operator-creation` | Done 2026-07-08 — `Operator.createOperator(Component)`, `addOperatorPackage`, and `ContainerOperator.createSubOperator` are an **intentional omission** (upstream has them): naming-convention reflection (`"<Class>Operator"` in registered packages via the public `(Component)` constructor) that no typed caller needed — the only production users cast the result to the type they already knew. Wrap components explicitly (`new JButtonOperator((JButton) waitSubComponent(...))`). Removing it also unpins the `(Component)` constructor from having to stay public, unblocking the planned factory-method construction redesign |
